...
//AddOn Theme
	$('.addOnThemeCheckBox').on('click', function(e){
		...
		if(infoThemeArr[3] == "false"){
			$(".appendThemeSelect").val(infoTheme);
			$(".appendNameThemeSelect").val(infoThemeArr[0] + " " + infoThemeArr[1]);
			$('#modal1').modal('toggle');
...
function getWorkContents(infoThemeArr) {
	$.ajax({
        ...
        success : function(data){ 
        	$(".workContentsCode").html("");
        	$(".workContentsCode").append('<option value = "">内容選択...</option>');
        	$.each(data, function(i,r){
        		var option = '<option value="'+r.workContentsCode+'">'+r.workContentsCode+' ['+r.workContentsCodeName+']'+'</option>';
        		$(".workContentsCode").append(option);
        	}) 
        },
		error: function() {
			console.log("エラー");
